...
## 2.4 해석 엔진(최종 확정): 3대 성격 자산 → 부하 센서 → 국면 질문

v2.0 리포트는 “성격을 낙인찍는 유형 분류”가 아니라, **지금의 학업·업무 운영을 돕는 동력 모델**로 설명합니다.

### 2.4.1 3대 성격 자산(IE/SA/IO)

- **IE(실행 리듬)**: “일단 시작해서 밀어붙이는 힘(착수·돌입·마감 압박에서 집중)”
- **SA(인지 양식)**: “계획·정리·기준을 잡아 효율을 만드는 힘(구조화·근거·절차)”
- **IO(에너지 배분/지향)**: “에너지를 사람·조율·상호작용에 두는지, 과업의 본질·깊이에 두는지”

> IE와 SA는 “완전한 직교”로 단정하지 않습니다. 대립 이분법도 아니지만, 데이터에서는 약한 음의 상관이 관찰될 수 있으므로 **‘보완적 기질’**로 기술합니다.

### 2.4.2 WD(부하 센서): IE(+)와 SA(-)의 주효과 중심

- **WD는 결함/낙인이 아니라**, “일이 몰릴 때 과부하 경고등이 켜지는 정도”입니다.
- 해석 원칙:
  - **SA는 과부하를 막는 방파제(보호 요인)**로 작동하기 쉽습니다.
  - **IE는 과부하 시 비상벨이 울리기 쉬운 신호**가 될 수 있습니다.

즉, 리포트는 “IE/SA/IO(자산) → WD(부하) → 다음 설계” 순으로 읽도록 구성합니다.

### 2.4.3 중앙형 트리아지 + 패러다임(29~33)

- IE·SA가 모두 중간대인 수검자는 1차로 **WD(안정 vs 과열)**로 트리아지합니다.
- 29~33번(쿤 패러다임)은 채점/점수화가 아니라 **성찰형 코칭 질문**으로 연결합니다.
  - “지금 방식이 편안하게 굴러가나(29)?”
  - “예전 방식이 잘 안 먹히는 신호가 있나(30)?”
  - “바꿔야 하는데 어디서부터 시작할지 답답한가(31)?”
  - “새 방법/도구를 실제로 써보고 있나(32)?”
  - “새 방식이 자리 잡아 성과/자신감이 오르는가(33)?”
